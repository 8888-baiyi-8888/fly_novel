...
用法：node dist/app/main.js [选项]
  ...
  --step n0|n1|n2|n3|n4|n5|n6|n7|n8  只跑单个节点（默认全链路 N0→N1→N2→(N3‖N4 并行)→N5→N6→N7→N8）
                           ...
                           n7：读 N1~N6 全部产物，生成书项目目录（纯程序，不调 LLM）
                           n8：验收书目录八类内容物 + AI 红线扫描消毒 + 写交接凭据（纯程序，不调 LLM）
  --force                N7 书目录已存在时强制重建（默认存在则跳过，保护手改的权威文件）
  --book <bookId>        N8 指定处理哪本书（缺省取 n7-workspace 下最新书目录）
...
real 模式全链路：N0 自主补全原始输入 → N1 生成创意草案 → N2 固化书籍配置
                 ...
                 → N7 书项目目录（持久化工作空间）→ N8 验收+红线消毒+交接凭据
  ...
  N7：持久化工作空间——把 N1~N6 全部产物序列化成书项目目录（纯程序，不调 LLM）：
      inkos.json（书配置+平台节奏尺子）/ story/*.md（人读权威，含 hooks.md 伏笔账本）
      / state/*.json（机器读权威）/ beats.json（节拍板）/ chapters、runtime、export（空目录待填）
  产物自动落盘：N0 文本 → artifacts/n0-raw-input/；N1 草案 → artifacts/n1-draft/；
                ...
                N6 State₀ → artifacts/n6-state0/；N7 书目录 → artifacts/n7-workspace/{bookId}/
