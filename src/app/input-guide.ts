...
【N0 原始输入 · 生产建议】
输入是自由文本，想写什么写什么。建议按下面 9 项组织，缺的可以不写：

1. 一句话定位：题材 + 类型（如：都市隐龙流爽文）
2. 主角：姓名、表面身份、真实身份、目标/执念
3. 世界观：世界是什么样的，有什么规则/隐藏设定
4. 核心卖点/爽点：读者爽在哪里（如：信息差、"轻描淡写的正确"）
5. 冲突/主线：主角要解决什么问题
6. 分卷规划：第一卷/第二卷/第三卷大致写什么
7. 创作约束：不能写什么（不虐主、不送女、反派不降智……）
8. 目标平台与篇幅：如番茄、100 章、每章 2500 字
9. 风格：如现实慢热、信息差、反差

推荐操作：把想法存成 txt 文件（UTF-8 编码），再运行：
  node dist/app/main.js --model real --file 你的想法.txt --output 草案.json
